...
//
// Primary sources:
//   Gold    → TwelveData (XAU/USD) │ Fallback: gold-api.com (XAU, no key)
//   BTC     → TwelveData (BTC/USD) │ Fallback: gold-api.com (BTC, no key)
//   DXY     → TwelveData (DXY)     │ Fallback: null (DXY basket is complex to reconstruct)
//   VIX     → Finnhub   (^VIX)     │ Fallback: null
...
//   "sourceStatus": { "fred": "ok", "twelvedata": "ok", "finnhub": "ok" },
...
import { fetchFRED, fetchTwelveDataQuote,
         fetchFinnhubQuote, fetchGoldAPIPrice }     from '../utils/fetchers.js';
...
export async function onRequest(context) {
  ...
  if (tdGold.status === 'fulfilled' && tdGold.value) {
    ...
    try {
      ...
      console.error('[/api/market] gold-api.com XAU fallback failed:', err.message);
    }
  }
  ...
  if (tdBTC.status === 'fulfilled' && tdBTC.value) {
    ...
    try {
      ...
      console.error('[/api/market] gold-api.com BTC fallback failed:', err.message);
    }
  }
